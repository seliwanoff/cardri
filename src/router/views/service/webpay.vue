...
export default {
  ...
  async mounted() {
    ...
    try {
      const gettransactions = await axios.get(
        `${process.env.VUE_APP_BASE_URL}api/gettransactions?type=36&year=${
          this.y
  ...
  methods: {
    async AprroveChinaPay(s, i) {
      ...
      await axios
        .post(`${process.env.VUE_APP_BASE_URL}api/verifyalipay`, data)
        .then(() => {
          //  console.log(res);
          try {
      ...
      if (this.day) {
        ...
        try {
          const getUsers = await axios.get(
            `${process.env.VUE_APP_BASE_URL}api/gettransactions?type=36&month=${this.am}&year=${this.y}&page=${this.pageNumber}`
          );
    ...
    async prev() {
      ...
      try {
        const getUsers = await axios.get(
          `${process.env.VUE_APP_BASE_URL}api/gettransactions?type=36&page=${this.pageNumber}&month=${this.am}&year=${this.y}`,
          {
            headers: {
              Authorization: "Bearer " + this.token,
            },
    ...
    async searchByCriteria() {
      try {
        const getUsers = await axios.get(
          `${process.env.VUE_APP_BASE_URL}api/gettransactions?type=36&search=${this.search}`
        );
        this.transactions = getUsers.data.data.data;
        this.totalpage = getUsers.data.data.total;
        this.per_page = getUsers.data.data.per_page;
    ...
    getMonthNumber: async function (m) {
      ...
      try {
        const getUsers = await axios.get(
          `${process.env.VUE_APP_BASE_URL}api/gettransactions?type=36&month=${this.am}&year=${this.y}`
        );
        this.transactions = getUsers.data.data.data;
        this.totalpage = getUsers.data.data.total;
        this.per_page = getUsers.data.data.per_page;
    ...
    async getDaysValue(day) {
      ...
      try {
        const getUsers = await axios.get(
          `${process.env.VUE_APP_BASE_URL}api/gettransactions?type=36&day=${this.day}&month=${this.am}&year=${this.y}`
        );
    ...
    async getYearTransact(year) {
      ...
      if (this.day) {
        ...
        try {
          const getUsers = await axios.get(
            `${process.env.VUE_APP_BASE_URL}api/gettransactions?type=36
...
<template>
  <Layout>
    <div class="row p-1 border-1 border bg-white">
      ...
      <div class="col" style="overflow: auto; position: relative">
        <div class="alert alert-primary" v-if="success == true">Successful</div>
        <div class="alert alert-danger" v-if="error == true">Ops! An error occur</div>
        <table v-if="transactions.length > 0" class="table">
          ...
          <tbody>
            <tr v-for="item in transactions" :key="item.id" scope="row">
              <td>{{ item.ref }}</td>
              <td>{{ item.reciever }}</td>
              ...
              <td v-else-if="item.status === '0'" style="text-alert">Failed</td>
              <td v-else-if="item.status === '2'" style="text-alert">Processing</td>
              <td style="text-alert" v-if="item.status === '2'">
                <div style="display: flex; gap: 8px">
                  <button
                    class="btn-confirm"
                    :disabled="alipay"
                    @click="AprroveChinaPay('1', item.id)"
                  >
                    Approve
                  </button>
                  <button
                    ...
                    @click="AprroveChinaPay('0', item.id)"
                  >
                    Decline
                  </button>
                </div>
              </td>
              <td style="text-alert" v-else>No Action</td>
            </tr>
          </tbody>
          <tfoot>
