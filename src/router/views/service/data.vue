...
export default {
  ...
  async mounted() {
    ...
    try {
      const gettransactions = await axios.get(
        `${process.env.VUE_APP_BASE_URL}api/gettransactions?type=2&year=${
          this.y
      ...
      this.page = Math.ceil(parseInt(this.getTotal / 20) + 1);
    } catch (e) {
  ...
  methods: {
    ...
    async searchByCriteria() {
      try {
        const getUsers = await axios.get(
          `${process.env.VUE_APP_BASE_URL}api/gettransactions?type=2&search=${this.search}`
        );
        this.transactions = getUsers.data.data.data;
        this.totalpage = getUsers.data.data.total;
        this.per_page = getUsers.data.data.per_page;
